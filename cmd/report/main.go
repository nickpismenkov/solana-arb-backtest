// Command report rolls up the executor ledgers (decisions.jsonl +
// trades.jsonl): decodable victims evaluated, profitable predictions, fires,
// landings, realized P&L, tips paid. Reads the JSONL the executor writes;
// safe to run while it's live.
//
...
func main() {
	...
	for _, d := range decisions {
		if s, ok := d["reason"].(string); ok && s == "profitable" {
			profitable++
		}
		if s, ok := d["reason"].(string); ok && s == "below_threshold" {
			below++
		}
