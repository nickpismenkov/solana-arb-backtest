// Package pools holds pool pair config + on-chain price decode. Byte
// offsets verified against mainnet (see git history / RUN.md). Same
// Q64.64 sqrt-price math for both venues — Orca Whirlpool and Raydium CLMM.
//
// The pair is env-configurable so the same tooling re-points at any pair
// that has a pool on both venues (defaults = liquid SOL/USDC). Env:
...
type PairCfg struct {
	...
	OrcaPool string
	RayPool  string
	// BaseMint: prices are quote-per-base on both venues.
	BaseMint string
	// BaseDec: the Whirlpool account doesn't carry decimals (Ray CLMM does).
	BaseDec    int32
	QuoteDec   int32
	OrcaFeeBps float64
	RayFeeBps  float64
	// RayVault0: Ray CLMM token_vault_0 — input vault == this ⇒ base is
	// being sold.
	RayVault0 string
	// BaseTokenProgram/QuoteTokenProgram: token program owning each mint
	// (classic SPL Token or Token-2022). A Token-2022 leg requires swapV2 +
	// Token-2022 ATAs. Default: both classic.
...
// NeedsSwapV2 is true if either side is Token-2022 → must use swapV2 +
// Token-2022 ATAs.
func (c *PairCfg) NeedsSwapV2() bool {
	return c.BaseTokenProgram == token2022 || c.QuoteTokenProgram == token2022
}
...
// Pair returns the lazily loaded pair config. Call after dotenv has run
// (all bins do).
func Pair() *PairCfg {
	cfgOnce.Do(func() {
		cfg = &PairCfg{
			...
			// Default Raydium CLMM SOL/USDC — the active 4bp pool; one
			// account holds sqrtPrice so it updates on every swap (no
			// vault-ratio lag).
			...
			BaseDec:           envInt32("BASE_DECIMALS", 9),
			QuoteDec:          envInt32("QUOTE_DECIMALS", 6),
			OrcaFeeBps:        envFloat("ORCA_FEE_BPS", 4.0),
			RayFeeBps:         envFloat("RAY_FEE_BPS", 4.0),
...
// u128LEAsF64 reads a little-endian u128 at offset o and returns it as a
// float64 (full 128-bit precision isn't needed — the callers immediately
// divide by 2^64 to recover a sqrt-price ratio).
func u128LEAsF64(d []byte, o int) float64 {
	lo := binary.LittleEndian.Uint64(d[o : o+8])
	hi := binary.LittleEndian.Uint64(d[o+8 : o+16])
	return float64(hi)*two64 + float64(lo)
}
...
func OrcaPrice(d []byte) (float64, bool) {
	...
	c := Pair()
	sqrt := u128LEAsF64(d, 65) / math.Pow(2, 64)
	mintA := mintAt(d, 101)
...
func RayClmmPrice(d []byte) (float64, bool) {
	...
	sqrt := u128LEAsF64(d, 253) / math.Pow(2, 64)
