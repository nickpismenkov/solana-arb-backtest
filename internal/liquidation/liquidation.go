// Package liquidation implements marginfi v2 liquidation engine — Stage 1:
// read-only health finder.
//
// It decodes on-chain Bank and MarginfiAccount state, computes each
...
// lending_account@72) confirmed on-chain. WrappedI80F48 is a 16-byte LE
// i128 fixed-point (value / 2^48), repr(C, align(8)).
package liquidation
...
// I80F48ToF64 converts a WrappedI80F48 (16-byte LE i128, 48 fractional bits)
// to float64.
func I80F48ToF64(bytes []byte) float64 {
	if len(bytes) < 16 {
		panic("liquidation: I80F48ToF64: need at least 16 bytes")
	}
	lo := binary.LittleEndian.Uint64(bytes[0:8])
	hi := binary.LittleEndian.Uint64(bytes[8:16])
	v := i128LEToFloat(lo, hi)
	return v / float64(uint64(1)<<48)
}

// i128LEToFloat interprets (lo, hi) as a little-endian signed 128-bit
// integer and returns its float64 value.
func i128LEToFloat(lo, hi uint64) float64 {
	// Treat hi as the signed high word.
	shi := int64(hi)
	if shi < 0 {
		// Negative: value = -( (^hi):(^lo) + 1 ) interpreted as 128-bit magnitude.
		// Compute two's complement negation across the 128-bit pair.
		nlo := ^lo + 1
		nhi := ^hi
		if nlo == 0 {
			nhi++
		}
		mag := float64(nhi)*18446744073709551616.0 + float64(nlo) // nhi * 2^64 + nlo
		return -mag
	}
	return float64(hi)*18446744073709551616.0 + float64(lo)
}

func readPubkey(data []byte, off int) solana.Pubkey {
	var b [32]byte
	copy(b[:], data[off:off+32])
	return solana.Pubkey(b)
}
...
// Bank holds the risk parameters we need from a Bank to price a position's
// health.
type Bank struct {
	Mint                 solana.Pubkey
	MintDecimals         uint8
	...
	// PriceUpdateV2 ref.
	OracleKey solana.Pubkey
	// OracleMaxAge: BankConfig.oracle_max_age (seconds) — the chain rejects
	// a price older than this with a stale-oracle error. VERIFIED @800:
	// USDC=300, wSOL=70, BONK=120. 0 = use the program default. We mirror
	// it so the finder doesn't over-flag on prices the chain considers
	// stale (SwitchboardStalePrice 6049).
	OracleMaxAge uint16
	// EmodeTag: this bank's emode class (0 = not emode-eligible as
	// collateral). VERIFIED @920: Amtw3n7G->619, USDC/other stables->57481.
	EmodeTag uint16
	// EmodeEntries: boosts this bank grants to collateral when borrowed as
	// a liability.
...
const (
	...
	emodeEntrySize   = 40
	// bankOracleMaxAge: BankConfig.oracle_max_age (u16 seconds) — VERIFIED
...
// DecodeBank decodes a Bank account. Returns nil, false if the data is too
// short or the discriminator doesn't match.
func DecodeBank(data []byte) (*Bank, bool) {
	if len(data) < 1864 || !discEqual(data, BankDisc) {
		return nil, false
	}
	...
	e := 0
	for bankEmodeEntries+(e+1)*emodeEntrySize <= len(data) && e < 16 {
		...
		if tag != 0 && init > 0.0 && init <= maint && maint <= 1.5 {
			emodeEntries = append(emodeEntries, EmodeEntry{CollateralTag: tag, AssetWeightInit: init, AssetWeightMaint: maint})
		}
		e++
	}
	b := &Bank{
...
// EffectiveAssetWeightMaint returns the effective maintenance asset weight
// for one collateral bank, applying marginfi's emode rule: emode boosts the
...
// MASize is the total size of a MarginfiAccount account.
const MASize = 2312
...
func DecodeMarginfiAccount(data []byte) (*MarginfiAccount, bool) {
	if len(data) < lendingAccount+maxBalances*balanceSize || !discEqual(data, MarginfiAccountDisc) {
		return nil, false
	}
	...
	for i := 0; i < maxBalances; i++ {
		...
		liabilityShares := I80F48ToF64(data[base+balLiabilityShares:])
...
func ActiveBankPks(data []byte) []solana.Pubkey {
	var v []solana.Pubkey
...
// DecodePriceUpdateV2 decodes a Pyth PriceUpdateV2 account -> (feedID,
// usdPrice, publishTime). Price is scaled by its exponent to whole-token
// USD.
func DecodePriceUpdateV2(data []byte) (feedID [32]byte, usdPrice float64, publishTime int64, ok bool) {
	if len(data) < 134 || !discEqual(data, PriceUpdateV2Disc) {
...
// Switchboard On-Demand PullFeed (owner SBondMDrcV3K..., 3208 bytes).
// marginfi oracle_setup 4 (SwitchboardPull) and 7 use these. The feed
// result is an i128 fixed-point (/ 1e18) at offset 56 — VERIFIED on-chain:
// SOL feed reads $80.98, a USDS feed $0.9999. disc =
// sha256("account:PullFeedAccountData")[..8].
...
func DecodeSwitchboardPull(data []byte) (float64, bool) {
	if len(data) < sbPullResult+16 || !discEqual(data, SwitchboardPullDisc) {
		return 0, false
	}
	lo := binary.LittleEndian.Uint64(data[sbPullResult : sbPullResult+8])
	hi := binary.LittleEndian.Uint64(data[sbPullResult+8 : sbPullResult+16])
	v := i128LEToFloat(lo, hi)
	usd := v / 1e18
...
// The Solana slot the current Switchboard result was produced at, offset 40
// in the PullFeedAccountData (VERIFIED empirically across all 1508 live
// feeds 2026-07-14: fresh feeds read ~350 slots behind head, dead feeds
// read 0/behind millions). marginfi gates liquidation on this via the
// bank's oracle_max_age; a result too far behind head reverts the
// liquidate ix with SwitchboardStalePrice (6049) — which is exactly what
// made ~6.3k accounts show as "liquidatable" to a finder that read the
// price but not its age.
const sbPullResultSlot = 40

// DecodeSwitchboardPullSlot decodes the result slot from a Switchboard
// On-Demand PullFeed account.
func DecodeSwitchboardPullSlot(data []byte) (uint64, bool) {
	if len(data) < sbPullResultSlot+8 || !discEqual(data, SwitchboardPullDisc) {
		return 0, false
	}
...
// only lets a stale account reach the sim gate, which rejects it (6049)
// and the caller backs it off. So we deliberately allow ~2x the chain's
// max-age before filtering — tight enough to kill the over-flagging (the
// old fixed 5000 was ~30x the chain's wSOL max-age), loose enough that a
// slot-rate mis-estimate never costs a fire.
const StaleSafetyFactor float64 = 2.0

// MaxStaleSlotsFor returns the per-bank Switchboard staleness ceiling in
// SLOTS, derived from the bank's on-chain oracle_max_age (seconds) so we
// mirror exactly what the program will accept — plus the safety factor.
...
// currentSlot as UNAVAILABLE (ok=false -> the health calc counts it
// missing and never trusts the account). Pyth feeds pass through unchanged
// (Pyth staleness is handled at the sponsored-feed/crank layer).
// currentSlot == 0 disables the gate (falls back to price-only).
func DecodeOraclePriceFresh(data []byte, currentSlot uint64, maxStaleSlots uint64) (float64, bool) {
...
type PriceMap = map[solana.Pubkey]float64

// BankMap maps bank pubkey -> decoded Bank.
type BankMap = map[solana.Pubkey]*Bank

// Health holds weighted assets/liabilities for maintenance health.
type Health struct {
	WeightedAssets      float64
	WeightedLiabilities float64
}

// Value returns the maintenance health value. Liquidatable when < 0.
func (h Health) Value() float64 {
	return h.WeightedAssets - h.WeightedLiabilities
}

// Liquidatable reports whether the account is liquidatable.
func (h Health) Liquidatable() bool {
	return h.Value() < 0.0
}

// Ratio returns weighted_liabs / weighted_assets — ratio >= 1.0 means
// underwater.
...
// HealthResult is the result of computing maintenance-weighted health for
// an account. Missing counts balances whose bank or price we couldn't
// resolve — if it's > 0 the health is INCOMPLETE and must NOT be trusted
// for a liquidation decision (skipping an unpriced collateral bank makes a
// healthy account look underwater).
...
func discEqual(data []byte, disc [8]byte) bool {
	if len(data) < 8 {
		return false
	}
